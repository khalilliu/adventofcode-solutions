...
TEST_INPUT2 = [["A", "D", "D", "B"], ["D", "B", "C", "C"], ["C", "A", "B", "B"], ["A", "C", "A", "D"]]
INPUT = [["D", "C"], ["D", "A"], ["B", "B"], ["A", "C"]]
INPUT2 = [["D", "D", "D", "C"], ["D", "B", "C", "A"], ["B", "A", "B", "B"], ["A", "C", "A", "C"]]
COSTS = {
    "A": 1,
    "B": 10,
    "C": 100,
...
def get_moves_to_bank(state: State, bank_size: int) -> Iterable[State]:
    for (hallway_index, amph) in enumerate(state.hallway):
        ...
        if hallway_index <= target_bank:
            is_path_clear = all(
                cell is None for cell in state.hallway[hallway_index + 1: target_bank + 1]
            )
        else:
            is_path_clear = all(
...
def get_moves_to_hallway(state: State, bank_size: int) -> Iterable[State]:
    for (bank_index, amphs) in state.banks.items():
        ...
        for hallway_index in VALID_HALLWAY_INDEXES:
            ...
            if is_path_clear:
                distance = int(abs(hallway_index - starting_index)) + 1 + (bank_size - len(amphs))
                used_energy = state.used_energy + (distance * COSTS[amph])
                hallway = list(state.hallway)
                ...
                if (
                    ...
                    if all(other_amph == amph for other_amph in state.banks[amph]):
                        ...
                        yield replace(state, used_energy=used_energy, hallway=state.hallway, banks=banks)
...
def get_state_key(state: State) -> object:
    return (state.used_energy, tuple(state.hallway), tuple((k, tuple(v)) for (k, v) in state.banks.items()))
    ...
    while q:
        ...
        if is_solved(state, bank_size=bank_size):
            if best_solution is None:
                ...
                best_solution = min(state, best_solution, key=lambda x: x.used_energy)
        elif best_solution is not None and state.used_energy >= best_solution.used_energy:
            continue
        ...
        for next_state in get_next_states(state, bank_size=bank_size):
            if DEBUG:
                ...
                for bank in next_state.banks.values():
                    for amph in bank:
                        counts[amph] += 1
                if counts != {"A": bank_size, "B": bank_size, "C": bank_size, "D": bank_size}:
